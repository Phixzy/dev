...
// Fetch subjects for this student's course and year level
$subjects = [];
$debug_info = [];

$debug_info[] = "=== SUBJECT FETCHING DEBUG ===";
$debug_info[] = "Student Info:";
$debug_info[] = "  - Student ID: " . $current_student['id'];
$debug_info[] = "  - Course: '" . $current_student['college_course'] . "'";
$debug_info[] = "  - Year Level: '" . $current_student['college_year'] . "'";

// Check ALL subjects in the database (not just distinct)
$all_subjects_sql = "SELECT id, subject_name, subject_code, course, year_level, instructor_name FROM subjects ORDER BY course, year_level, subject_name";
$all_subjects_result = $conn->query($all_subjects_sql);
if ($all_subjects_result && $all_subjects_result->num_rows > 0) {
    $debug_info[] = "All subjects in DB (" . $all_subjects_result->num_rows . " total):";
    while ($row = $all_subjects_result->fetch_assoc()) {
        $match_status = "";
        // Check if this would match the student
        if ($row['course'] == $current_student['college_course'] && $row['year_level'] == $current_student['college_year']) {
            $match_status = " [✓ WOULD MATCH]";
        }
        $debug_info[] = "  - " . $row['subject_name'] . " (Code: " . $row['subject_code'] . ") | Course: '" . $row['course'] . "' | Year: '" . $row['year_level'] . "' | Instructor: " . $row['instructor_name'] . $match_status;
    }
} else {
    $debug_info[] = "No subjects found in DB at all!";
}

// Main query to fetch subjects for this student
$subject_sql = "SELECT id, subject_name, subject_code, course, year_level, hour, instructor_name 
                FROM subjects 
                WHERE course = ? AND year_level = ?
                ORDER BY subject_name";
$debug_info[] = "";
$debug_info[] = "=== EXECUTING QUERY ===";
$debug_info[] = "Query: " . $subject_sql;
$debug_info[] = "Binding: course='" . $current_student['college_course'] . "', year='" . $current_student['college_year'] . "'";

$stmt = $conn->prepare($subject_sql);
if (!$stmt) {
    $debug_info[] = "ERROR: Failed to prepare statement: " . $conn->error;
} else {
    $stmt->bind_param("ss", $current_student['college_course'], $current_student['college_year']);
    if (!$stmt->execute()) {
        ...
        if (!$subject_result) {
            $debug_info[] = "ERROR: Get result failed: " . $stmt->error;
        } else {
            $debug_info[] = "Found " . $subject_result->num_rows . " subjects matching student criteria";
            
            if ($subject_result->num_rows > 0) {
                while ($row = $subject_result->fetch_assoc()) {
                    $subjects[] = $row;
                    $debug_info[] = "  ✓ LOADED: " . $row['subject_name'] . " (Code: " . $row['subject_code'] . ") | Instructor: " . $row['instructor_name'];
                }
            } else {
                $debug_info[] = "";
                $debug_info[] = "=== POSSIBLE MISMATCH ===";
                $debug_info[] = "No subjects found for your exact course/year combination!";
                $debug_info[] = "";
                $debug_info[] = "Your course: '" . $current_student['college_course'] . "'";
                $debug_info[] = "Your year: '" . $current_student['college_year'] . "'";
                $debug_info[] = "";
                $debug_info[] = "To fix this, please:";
                $debug_info[] = "1. Run: http://localhost/dev/admin_page/fix_student_subjects.php";
                $debug_info[] = "2. Or ask admin to update your course/year in students table";
                $debug_info[] = "Or update subjects table to match your course/year";
            }
        }
    }
    $stmt->close();
...
$_SESSION['subject_debug'] = $debug_info;
...
<body>
    ...
    <nav>
        ...
        <div class="menu-items">
            <ul class="nav-links">
                <li><a href="studentpage.php" class="active">
    ...
    <section class="dashboard">
        ...
        <div class="activity">
            <div class="title">
                ...
                <span class="text">My Subjects - <?php echo htmlspecialchars($current_student['college_course'] . ' | ' . $current_student['college_year']); ?></span>
            </div>
                    ...
                    <table class="student-table" id="studentTable">
                        ...
                        <tbody>
                            <?php if (empty($subjects)): ?>
                                <tr>
                                    <td colspan="6" class="no-data" style="text-align: center; color: #6c757d; font-style: italic; padding: 50px 20px;">
                                        <i class="fas fa-book-open" style="font-size: 3rem; color: #dee2e6; margin-bottom: 1rem;"></i>
                                        <p>No subjects available for your course (<?php echo htmlspecialchars($current_student['college_course']); ?>) and year level (<?php echo htmlspecialchars($current_student['college_year']); ?>)</p>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($subjects as $subject): ?>
                                    <tr data-subject="<?php echo htmlspecialchars($subject['subject_code']); ?>">
                                        <td>
                                        ...
                                        <td>
                                            <div style="display: flex; align-items: center; gap: 10px;">
                                                ...
                                                <span><?php echo htmlspecialchars($subject['instructor_name']); ?></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="hours-badge">
                                                <i class="fas fa-clock" style="margin-right: 5px;"></i>
                                                <?php echo htmlspecialchars($subject['hour']); ?> hrs
                                            </span>
                                        </td>
                                        <td>
